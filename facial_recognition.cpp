...
int main(int argc, char **argv)
{
  ...
  for(int a = 1; a <26; a++)
  {
    ...
    file_name = "src/facial_recognition/people/CONFUSION/";
...
void ImageReceivedCallback2(Mat frame)
{
  static int face_counter = 0;                // Used for saving training images
  ...
  for( size_t i = 0; i < faces.size(); i++ )
  {
    ...
    if(size < 65) continue; // Disregard anomalies and people who are far away
    ...
    // Code for saving training images - NORMALLY COMMENTED OUT
    ...
    // Replace the entry in recent_locations if current size is above minimum and is bigger
    if(size > 65 && size > biggest)
    {
      biggest = size;
  ...
  } // Until all faces in this image have been checked
